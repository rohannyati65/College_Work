...
int main()
{
	...
	switch(subnet)
	{
		case 8:
                  ...
                  printf("\nSUBNET MASK : %d.%d.%d.%d\n",x,y,y,y);
				  printf("\nNETWORK ID : %d.%d.%d.%d\n",a&x,b&y,c&y,d&y); //Bitwise AND between IP address and subnet mask
				  break;

		case 16:
                  printf("\nBROADCAST ADDRESS : %d.%d.255.255\n",a,b);
				  printf("\nSUBNET MASK : %d.%d.%d.%d\n",x,x,y,y);
				  printf("\nNETWORK ID : %d.%d.%d.%d",a&x,b&x,c&y,d&y); //Bitwise AND
				  break;

		case 24:
                  printf("\nBROADCAST ADDRESS : %d.%d.%d.255\n",a,b,c);
				  printf("\nSUBNET MASK : %d.%d.%d.%d\n",x,x,x,y);
				  printf("\nNETWORK ID : %d.%d.%d.%d",a&x,b&x,c&x,d&y); //Bitwise AND
				  break;
	}
    return 0;
}
